...
class Clients
{
	...
	function __construct( $clave, $nombres, $apellidos, $mail, $PostalCode, $domicilio, $celular, $ReferredBy)
	{
		 $this->ID = $clave;
	...
	public function ConvertToSOAP()
	{
		...
		$arr['AddressLine1'] = $this->AddressLine1;
		$arr['MobilePhone'] = $this->MobilePhone;
			$arr['ReferredBy'] = $this->ReferredBy;
		return $arr;
	}
}
//////////////
...
class MBAPIService
{
	...
	public function SetDefaultClients(Clients $Clients)
	{
		$this->defaultClients = $Clients;
	}
...
//////////
protected function GetCliente(clients $clients)
{
	if (isset($clients))
	{
		return $clients;
	}
	else if (isset($this->DefaultClients))
	{
		return $this->DefaultClients;
	}
	else
	{
		throw new Exception('verga');
	}
}
}
